...
- 3.将⼤的请求拆解(对于请求的资源数量来讲，服务器已开启对http2的⽀持，其“多路复⽤”的特性致使，请求的⽂件数量多少不会对请求的总时间有较⼤影响, 但是可以缓存不必重复下载的资源)
...
尽管 JavaScript 是单线程的，但是它能够处理异步操作，如 AJAX 请求、Promise、setTimeout 等。这是因为 JavaScript 引擎并不是独立运行的，它运行在宿主环境中，通常这个宿主环境是浏览器或 Node.js。

当我们说 JavaScript 是单线程的，我们主要是指 JavaScript 的执行模型是单线程的。也就是说，在任何时刻，JavaScript 只能执行一段代码，而其他的代码必须等待。

然而，JavaScript 的宿主环境并不是单线程的。例如，在浏览器中，有许多其他线程在同时运行，如处理用户输入、发送网络请求、定时器等。这些任务可以并行进行，而且一旦完成，它们可以将回调函数放入 JavaScript 的任务队列中等待执行。

Promise.all 方法的工作原理，就利用了这个机制。当你传递一个 Promise 数组给 Promise.all，这些 Promise 通常会基于网络请求或其他异步操作创建。这些网络请求是由浏览器并行处理的，而不是由 JavaScript 单线程处理的。一旦所有的 Promise 都完成（或者有一个被拒绝），Promise.all 就会将一个回调函数添加到任务队列中，等待执行。

这也是为什么 Promise.all 能同时处理多个请求的原因。实际上，这并不是 JavaScript 自身的能力，而是 JavaScript 所运行的环境提供的能力
